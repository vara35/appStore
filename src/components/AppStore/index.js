...
class AppStore extends Component {
  state = {userApps: tabsList[0].tabId, searchInput: ''}

  updateState = () => {
    const {userApps} = this.state
    const updatedState = appsList.filter(
      eachOne => eachOne.category === userApps,
    )
    return updatedState
  }

  updateByUSer = tabId => {
    this.setState({userApps: tabId})
  }

  userInput = event => {
    const getUserInput = event.target.value.toLowerCase()
    this.setState({searchInput: getUserInput})
  }

  render() {
    const getDataFromState = this.updateState()
    const {searchInput} = this.state
    const searchResult = getDataFromState.filter(eachOne =>
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="ul-list">
          {tabsList.map(eachOneFirst => (
            <TabItem
              tabItem={eachOneFirst}
              key={eachOneFirst.tabId}
              updateByUSer={this.updateByUSer}
            />
          ))}
        </ul>
        <ul className="ul-container">
          {searchResult.map(eachApp => (
            <AppItem appList={eachApp} key={eachApp.appId} />
          ))}
        </ul>
      </div>
    )
